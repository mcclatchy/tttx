...
*/
		...
		var epNumber = playerList.length - deepLinkIndex -1;	// Also figure out the episode number.
		
	    var secondHalf = playerList.slice((deepLinkIndex+1))   									// break to second half (4,5,6)
		var firstHalf = playerList.slice(0,(deepLinkIndex+1));									// break to first half (1,2,3)
		
	
		console.table(firstHalf.reverse());	
		console.table(secondHalf.reverse());
	
				
		console.log(deepLinkIndex);
		playerList = firstHalf.concat(secondHalf);											// join to loop (4,5,6,1,2,3)
		console.table(playerList);


	}else{													// Else There is no Deeplink so keep the playerlist as is.
		playerList = originalList;
		epNumber = originalList.length-1;
		
//																			NEW 12-20		
		originalList.push(originalList[0]);								// In reverse cron add the most recent episode at the end (5,4,3,2,1 +[,5])
		originalList.reverse();											// Flip to Chron (5,1,2,3,4,5,)
//																			NEW 12-20		
...
//Build the Visual Playlist	
	...
	ttPlayerInstance.on('ready',function(){
		var playlist = visualPlaylist.reverse();
...
//Build the Current Title and Description
		...
		if (epNumber != 0){
				var hero_ep = 'Episode ' + Number(epNumber) ;
			}else{
				var hero_ep = 'Trailer';
			}
